...
### 使用方法
1. **外部配置文件（可选）**: 如果需要自定义配置，可以将配置文件推送到 `/data/audio_configs.json`
2. **应用启动**: 应用会自动检测外部配置文件是否存在
3. **配置加载**: 如果存在外部配置，使用外部配置；否则使用内置的默认配置
4. **配置切换**: 在应用中点击"配置"按钮选择不同配置，点击"重新加载配置文件"刷新外部配置
